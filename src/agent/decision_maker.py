...
class TradingAgent:
    ...
    def _decide(self, context, assets):
        ...
        system_prompt = (
            ...
            "- x_observations providing raw observations from X/Twitter posts about each asset (when available)\n"
            "- market_context providing broader crypto market news, regulatory events, and macro factors (when available)\n\n"
            ...
            "Market Context (when available, purely observational):\n"
            "• regulatory_events: Government actions, legal developments affecting crypto broadly\n"
            "• institutional_activity: ETF flows, corporate treasury moves, exchange developments\n"
            "• macro_factors: Fed policy, global markets, risk-on/risk-off shifts\n"
            "• market_sentiment: Overall crypto market mood\n"
            "• notable_events: Major news affecting all cryptocurrencies\n"
            "• risk_factors: Systemic risks or contagion concerns\n\n"
            "Order Book (when available, purely observational):\n"
            "• bid_ask_imbalance: Ratio of bid to ask liquidity (>1 = more buyers, <1 = more sellers)\n"
            "• total_bid_liquidity: Total size of buy orders in top levels\n"
            "• total_ask_liquidity: Total size of sell orders in top levels\n"
            "• bid_walls: Large buy orders acting as support (price, size)\n"
            "• ask_walls: Large sell orders acting as resistance (price, size)\n"
            "• spread_pct: Bid-ask spread as percentage\n\n"
            "Trade Flow (when available, purely observational):\n"
            "• buy_sell_ratio: Volume ratio of executed buys to sells from recent trades\n"
            "• buy_volume_pct/sell_volume_pct: Percentage breakdown of buy vs sell volume\n"
            "• num_buys/num_sells: Count of buy vs sell trades\n"
            "• large_buys/large_sells: Whale-sized trades (5x+ average size) with price and size\n"
            "• avg_trade_size: Average trade size in recent activity\n"
            "Note: Buys = aggressive buyers hitting asks, Sells = aggressive sellers hitting bids\n\n"
            "Funding History (when available, purely observational):\n"
            "• current_rate: Most recent funding rate\n"
            "• avg_rate: Average funding rate over lookback period\n"
            "• min_rate/max_rate: Range of funding rates\n"
            "• trend: Whether funding is accelerating (getting more extreme), decelerating, or stable\n"
            "• deviation_from_avg_pct: How far current funding is from average (signals crowding)\n"
            "• avg_premium: Average perp-spot premium (positive = perp trading above spot)\n\n"
